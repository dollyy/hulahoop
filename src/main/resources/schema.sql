...
SELECT s.id, s.name AS strategyName, s.duration, s.for_num AS forNum, s.collect_num AS collectNum, s.content, city_id, c.name AS cityName, u.username
FROM strategies s JOIN cities c ON s.city_id=c.id JOIN users u ON s.user_id=u.id
WHERE user_id = 2;
...
FROM comments c JOIN users u ON c.user_id=u.id WHERE c.strategy_id=5 ORDER BY level;

SELECT u.id,u.username FROM comments c JOIN users u ON c.user_id=u.id WHERE level='1.1';

SELECT id, user_id, name, city_id, duration, content, main_img, for_num, collect_num, DATE_FORMAT(create_time,
...
INSERT INTO feedback_info(user_id, content, level, parent, sequence, create_time)
VALUES (2,'你好呀',1,0,1,'2018-02-27 12:23:45'),(3,'首次登录',2,0,1,'2018-02-27 12:23:46'),
  (1,'hi!',1.2,1,2,'2018-02-27 12:23:47'),(1,'欢迎你',2.2,2,2,'2018-02-27 12:23:48'),
  (2,'o(∩∩)o...哈哈',1.3,1,3,'2018-02-27 12:23:49');
...
-- 查看某条反馈的全部信息
SELECT * FROM feedback_info WHERE level LIKE '4%';
